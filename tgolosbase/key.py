...
class Key():
	...
	def get_private_from_secret(self, secret):
		...
		private_key = str(base58CheckEncode(0x80, k))
		return private_key
	...
	def get_raw_public(self, key):
		return key.replace(self.prefix, '', 1)
		
	def get_raw_private(self, key):
		return key.replace('5', '', 1)
		
	def get_overlap(self, wif):
		private_key = self.get_raw_private(wif)
		public_key = self.get_raw_public(self.get_public_from_private(wif))
		overlap = []
		for i in range(50):
			if private_key[i] == public_key[i]:
				overlap.append(private_key[i])
		return(''.join(overlap))
